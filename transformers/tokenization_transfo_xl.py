...
class TransfoXLCorpus(object):
    ...
    def build_corpus(self, path, dataset):
        ...
        if self.dataset in ["ptb", "wt2", "enwik8", "text8"]:
            ...
            self.vocab.count_file(os.path.join(path, "train_with_summary.txt"))
        ...
        if self.dataset in ["ptb", "wt2", "wt103"]:
            ...
            self.test = self.vocab.encode_file(os.path.join(path, "train_with_summary.txt"), ordered=True)
        elif self.dataset in ["enwik8", "text8"]:
            self.train = self.vocab.encode_file(os.path.join(path, "train.txt"), ordered=True, add_eos=False)
            self.valid = self.vocab.encode_file(os.path.join(path, "valid.txt"), ordered=True, add_eos=False)
            self.test = self.vocab.encode_file(os.path.join(path, "train_with_summary.txt"), ordered=True, add_eos=False)
        elif self.dataset == "lm1b":
            self.train = train_paths
            self.valid = self.vocab.encode_file(os.path.join(path, "valid.txt"), ordered=False, add_double_eos=True)
            self.test = self.vocab.encode_file(os.path.join(path, "train_with_summary.txt"), ordered=False, add_double_eos=True)
